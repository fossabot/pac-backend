...
func main() {
	...
	// Handler chains
	jsonChain := alice.New(middleware.CORSMiddleware, middleware.EnforceJsonContentType)
	secureChain := alice.New(middleware.CORSMiddleware)
	if cnf.OAuthEnable {
	...
	sm.Handle("/locations/{id:[0-9]+}", secureChain.Then(secureChain.Then(http.HandlerFunc(lh.GetLocation)))).Methods("GET")
	...
	sm.Handle("/locations/{id:[0-9]+}", secureChain.Then(secureChain.Then(http.HandlerFunc(lh.DeleteLocation)))).Methods("DELETE")
	// Events
	...
	sm.Handle("/events/{id:[0-9]+}", secureChain.Then(http.HandlerFunc(eh.GetEvent))).Methods("GET")
	...
	sm.Handle("/organizations/{id:[0-9]+}", secureChain.Then(http.HandlerFunc(oh.GetOrganization))).Methods("GET")
	...
	sm.Handle("/persons/{id:[0-9]+}", secureChain.Then(http.HandlerFunc(ph.GetPerson))).Methods("GET")
	...
	sm.Handle("/rooms/{id:[0-9]+}", secureChain.Then(http.HandlerFunc(rh.GetRoom))).Methods("GET")
	...
	sm.Handle("/topics/{id:[0-9]+}", secureChain.Then(http.HandlerFunc(th.GetTopic))).Methods("GET")
	...
	sm.Handle("/talks/{id:[0-9]+}", secureChain.Then(http.HandlerFunc(tkh.GetTalk))).Methods("GET")
	...
	sm.Handle("/talkDates/{id:[0-9]+}", secureChain.Then(http.HandlerFunc(tdh.GetTalkDate))).Methods("GET")
